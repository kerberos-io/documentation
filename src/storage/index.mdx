...
1. One or more Kerberos Enterprise agents, can store their data directly on [Kerberos Cloud](/cloud).
2. You have the option to Bring Your Own Storage (BYOS), using Kerberos Storage. Kerberos Storage allows you to configure the storage provider you want. This could be a storage provider in the cloud such as AWS S3, GCP storage or Azure Blob store. Next to that Kerberos Storage can also be installed on premise (at the edge), and use storage providers such as Minio.
