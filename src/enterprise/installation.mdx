...
First clone the configrations from our Github repo.
    ...
    git clone https://github.com/kerberos-io/enterprise enterprise-factory
...
When you've installed the Helm client, you have to verify if it's **version 2 or 3**.

For Helm v2 you need to initiate Helm first (also called the Tiller service) inside your cluster.
...
Execute the `helm install` command. If using Helm version to write `--name traefik`.
