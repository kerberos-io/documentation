...
menu: Enterprise
route: /enterprise/installation-cloud
---
...
First clone the configrations from our Github repo.
    ...
    git clone https://github.com/kerberos-io/enterprise enterprise
    cd enterprise
...
Add Helm repository and install traefik.
    ...
    helm install traefik -f ./yaml/traefik/values.yaml stable/traefik
...
Have a look into the `yaml/mongodb/values.yaml` file, you will find plenty of configurations for your MongoDB instance. You will also find the attribute where you can change the root password of mongodb. If using Helm version 2, write `--name mongodb`.

    helm repo add bitnami https://charts.bitnami.com/bitnami
    helm install mongodb bitnami/mongodb --values ./yaml/mongodb/values.yaml
...
The last step is to install the factory service. Kerberos Enterprise is managed through an application which we call the Factory. It is responsible for initiating the deployments inside your cluster. These deployments is what we also call (similar to the Open Source version) the machinery.
...
Before installing the Factory service, open the `yaml/factory/deployment.yaml` configuration file. At the bottom file you will find two endpoints, similar to the traefik config file. Update the domain names to your own domain, and add these to your DNS server (pointing to the same IP as the traefik EXTERNAL-IP).
